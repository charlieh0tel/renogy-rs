...
const BMS_ADDRESSES: [u8; 4] = [0x30, 0x31, 0x32, 0x33];

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let args: Vec<String> = env::args().collect();

    let mac_address = if args.len() > 1 {
        args[1].clone()
    } else {
        println!("Discovering BT-2 devices...");
        let devices = discover_bt2_devices().await?;
        if devices.is_empty() {
            eprintln!("No BT-2 devices found. Usage: bt2-query <MAC_ADDRESS>");
            std::process::exit(1);
        }
    ...
    let adapter = env::var("BT_ADAPTER").unwrap_or_else(|_| "hci0".to_string());
    println!("Connecting to {} via {}...", mac_address, adapter);

    let mut transport = Bt2Transport::connect_by_address(&mac_address, &adapter).await?;
    println!("Connected!\n");

    println!("Scanning for batteries...\n");

    for addr in BMS_ADDRESSES {
